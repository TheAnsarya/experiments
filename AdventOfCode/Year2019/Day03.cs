...
namespace AdventOfCode.Year2019 {
	...
	class Day03 {
		...
		private List<Segment> GetWire(string directions) {
			var wire =
				directions
					.Split(',')
					.Select(x => new {
						Direction = x[0],
						Length = int.Parse(x.Substring(1, x.Length - 1))
					})
					.Aggregate(new List<Point>(), (a, b) => {
						if (a.Count == 0) {
							a.Add(Point.Zero.Next(b.Direction, b.Length));
						} else {
							a.Add(a[a.Count - 1].Next(b.Direction, b.Length));
						}
						return a;
					})
					.Aggregate(new List<Segment>(), (a, b) => {
						if (a.Count == 0) {
							a.Add(new Segment(Point.Zero, b));
						} else {
							a.Add(new Segment(a[a.Count - 1].Two, b));
						}
						return a;
					});
		...
		public string PartTwo() {
			...
			foreach (var segmentA in wires[0]) {
				...
				foreach (var segmentB in wires[1]) {
					var crosses = segmentA.CrossesAt(segmentB);
					if ((crosses != null) && (crosses.Distance != 0)) {
						var currentSteps = stepsA + new Segment(segmentA.One, crosses).Distance + stepsB + new Segment(segmentB.One, crosses).Distance;
						if (currentSteps < shortest) {
							shortest = currentSteps;
						}
					}

					stepsB += segmentB.Distance;
				}

				stepsA += segmentA.Distance;
			}
		...
		private class Point {
			...
			public Point Next(char direction, int length) {
				if (direction == 'U') {
		...
		private class Segment {
			...
			// Is only vertical or horizontal
			public bool IsVertical() => One.X == Two.X;

			// Can ignore same direction except for the ends (no overlap, only touch)
			public Point? CrossesAt(Segment other) {
				if (this.IsVertical()) {
					if (other.IsVertical()) {
						if (this.One.Equals(other.One) || this.One.Equals(other.Two)) {
							return this.One;
						} else if (this.Two.Equals(other.Two)) {
							return this.Two;
						}
					} else {
						var lowX = Math.Min(other.One.X, other.Two.X);
						var highX = Math.Max(other.One.X, other.Two.X);
						var lowY = Math.Min(this.One.Y, this.Two.Y);
						var highY = Math.Max(this.One.Y, this.Two.Y);
						if ((lowX < this.One.X) && (highX > this.One.X) && (lowY < other.One.Y) && (highY > other.One.Y)) {
							return new Point(this.One.X, other.One.Y);
						}
					}
				} else {
					if (other.IsVertical()) {
						var lowX = Math.Min(this.One.X, this.Two.X);
						var highX = Math.Max(this.One.X, this.Two.X);
						var lowY = Math.Min(other.One.Y, other.Two.Y);
						var highY = Math.Max(other.One.Y, other.Two.Y);
						if ((lowX < other.One.X) && (highX > other.One.X) && (lowY < this.One.Y) && (highY > this.One.Y)) {
							return new Point(other.One.X, this.One.Y);
						}
					} else {
						if (this.One.Equals(other.One) || this.One.Equals(other.Two)) {
							return this.One;
			...
			public int Distance => IsVertical() ? Math.Abs(One.Y - Two.Y) : Math.Abs(One.X - Two.X);
		}
	}
}
